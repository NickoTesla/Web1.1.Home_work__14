...
async def login(body: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    :param body: OAuth2PasswordRequestForm: Pass the username and password to the function
...
async def signup(body: UserModel, background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)):
    ...
    :param body: UserModel: Pass the user's information to the function
    :param background_tasks: BackgroundTasks: Add a task to the background queue
    :param request: Request: Get the base url of the server
    :param db: Session: Get the database session
    :return: A dict, but the response is a json
    :doc-author: Trelent
    """
    exist_user = await repository_users.get_user_by_email(body.email, db)
    if exist_user:
...
async def request_email(body: RequestEmail, background_tasks: BackgroundTasks, request: Request,
    ...
    :return: A message depending on whether the user is already confirmed or not
    :doc-author: Trelent
    """
